...
if __name__ == "__main__":
    ...
    with open(f"{LEGGED_GYM_ROOT_DIR}/deploy/deploy_mujoco/configs/{config_file}", "r") as f:
        ...
        policy_path = config["policy_path"].replace("{LEGGED_GYM_ROOT_DIR}", LEGGED_GYM_ROOT_DIR) + "Pick.pt"
    ...
    # Load robot model
    m = mujoco.MjModel.from_xml_path(xml_path+"scene_full_pick.xml")
    d = mujoco.MjData(m)
    ...
    with mujoco.viewer.launch_passive(m, d) as viewer:
        ...
        counter = 0
        d.qpos[7:-7] = default_angles
        d.qvel[6:-6] = 0.
        d.qpos[:3] = np.array([0., 0., 0.883])
        d.qpos[3:7] = np.array([1., 0., 0., 0.])
        d.qvel[:6] = 0.
        d.qpos[-7:] = np.array(rel_pose_object) + np.array([0.015, 0.015, 1., 0., 0., 0., 0.])
        mujoco.mj_step(m, d)
        ...
        while viewer.is_running() :
            ...
            if counter*simulation_dt > 7. :
                ...
                d.qpos[3:7] = np.array([1., 0., 0., 0.])
                d.qvel[:6] = 0.
            tau = pd_control(target_dof_pos, d.qpos[7:-7][active_joint_ids], kps[active_joint_ids], np.zeros_like(kds[active_joint_ids]), d.qvel[6:-6][active_joint_ids], kds[active_joint_ids])
            tau_left_hand = pd_control(target_dof_left_hand, d.qpos[7:-7][left_hand_joint_ids], kps[left_hand_joint_ids], np.zeros_like(kds[left_hand_joint_ids]), d.qvel[6:-6][left_hand_joint_ids], kds[left_hand_joint_ids])
            tau_right_hand = pd_control(target_dof_right_hand, d.qpos[7:-7][right_hand_joint_ids], kps[right_hand_joint_ids], np.zeros_like(kds[right_hand_joint_ids]), d.qvel[6:-6][right_hand_joint_ids], kds[right_hand_joint_ids])
            d.ctrl[:] = 0.
            ...
            if counter % control_decimation == 0:
                ...
                # create observation
                qj = d.qpos[7:-7][active_joint_ids]
