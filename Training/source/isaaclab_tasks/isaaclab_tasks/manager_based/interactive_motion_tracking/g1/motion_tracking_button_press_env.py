...
def reset_button(
    ...
    if hasattr(env, 'start_motion_times'):
        ...
        env.last_contacts[env_ids, :] = 0.
        offset = motion_res["offsets"]
        goal_pos = motion_res["grab_pos"]
        if "buttons" in env.scene.keys():
            buttons: RigidObject = env.scene["buttons"]
            buttons_pos = goal_pos + offset + env.scene.env_origins[env_ids]
            buttons_pos[:, 2] -= 0.068  # raise the buttons a bit above the ground
            buttons_pos[:, 0] += 0.03  # move the buttons a bit forward
            buttons_quat = torch.zeros((env.scene.num_envs, 4), device=env.device)[env_ids]
            buttons_quat[:, 0] = 1.0  # set the w component to 1.0 for identity quaternion
            buttons.write_root_pose_to_sim(torch.cat([buttons_pos, buttons_quat], dim=-1), env_ids=env_ids)
            buttons.write_root_velocity_to_sim(velocities, env_ids=env_ids)

@configclass
class EventCfg(EventCfgBase):
    reset_buttons = EventTerm(
        func=reset_button,
        mode="reset"
    )
...
class G1Rewards(G1RewardsBase):
    ...
    if TASK_DENSE:
        ...
        goal_approach_reward = RewTerm(func=goal_approach_reward,
            params={"asset_cfg": SceneEntityCfg("robot", body_names=["right_wrist_yaw_link"]), "offset_x": 0.21, "offset_y": 0.0, "offset_z": 0.03},
            weight=100.0,
            )
...
@configclass
class G1ButtonPressEnvCfg(G1InteractiveBaseEnvCfg):
...
@configclass
class G1ButtonPressEnvPlayCfg(G1InteractiveBaseEnvCfg):
    rewards: G1Rewards = G1Rewards()
    events: EventCfg = EventCfg()
    observations: ObservationsCfg = ObservationsCfg()
    scene: MySceneCfg = MySceneCfg(num_envs=8192, env_spacing=2.5)
    ...
    def __post_init__(self):
        ...
        rot_before = torch.tensor([[0.69529, 0.65952, -0.19659, -0.20725]])
        self.scene.buttons = ArticulationCfg(
            prim_path="{ENV_REGEX_NS}/buttons",
            spawn=sim_utils.UsdFileCfg(
                usd_path="assets/Collected_ElevatorLobby/buttons.usd",
                activate_contact_sensors=False,
            ),
            actuators={
                "button_up": ImplicitActuatorCfg(
        ...
        self.scene.camera = CameraCfg(prim_path="{ENV_REGEX_NS}/Camera_new",
                                      ...
                                      offset=CameraCfg.OffsetCfg(
                                          pos=(-2+2.14-1.629, -5.94+1.04, 1.06841),
