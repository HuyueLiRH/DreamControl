...
def open_door_bonus(env: ManagerBasedRLEnv, asset_cfg: SceneEntityCfg) -> torch.Tensor:
    ...
    The bonus is given when the door is open.
    """
    ...
    if env.num_envs < 1001 :
        env.n_successes += (door_pos>0.05) * motion_res["is_closed"]
    
    reward = (door_pos>0.05) * motion_res["is_closed"].float()   # clamp the position to [0, 1]
    return reward
...
class ActionsCfg(ActionsCfgBase):
    ...
    right_hand_action = mdp.BinaryJointPositionActionCfg(asset_name="robot",
            ...
            open_command_expr={"right_hand_index_0_joint": 0, 
                                ...
                                "right_hand_index_1_joint": np.pi/4.,
                                "right_hand_middle_1_joint": np.pi/4.,
...
@configclass
class G1OpenDoorEnvCfg(G1InteractiveBaseEnvCfg):
