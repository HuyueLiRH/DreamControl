...
class G1Rewards():
    ...
    if TASK_DENSE:
        ...
        goal_approach_reward = RewTerm(func=object_approach_reward_right,
            params={"asset_cfg": SceneEntityCfg("robot", body_names=["right_wrist_yaw_link"]), "offset_x": 0.14, "offset_y": 0.03},
            weight=100.0)
...
@configclass
class G1PunchEnvCfg(G1InteractiveBaseEnvCfg):
...
@configclass
class G1PunchPlayEnvCfg(G1InteractiveBaseEnvCfg):
    rewards: G1Rewards = G1Rewards()
    observations: ObservationsCfg = ObservationsCfg()
    scene: MySceneCfg = MySceneCfg(num_envs=8192, env_spacing=2.5)
    terminations: TerminationsCfg = TerminationsCfg()
    ...
    def __post_init__(self):
        ...
        rot_before = torch.tensor([[0.35957, 0.30891, 0.57378, 0.66788]])
        ...
        self.scene.camera = CameraCfg(prim_path="{ENV_REGEX_NS}/Camera_new",
                                      ...
                                      offset=CameraCfg.OffsetCfg(
                                          pos=(-9.02*0.7+2.1+2.75776*0.7, -17.1*0.7+13.35736*0.7, 2.53*0.7),
