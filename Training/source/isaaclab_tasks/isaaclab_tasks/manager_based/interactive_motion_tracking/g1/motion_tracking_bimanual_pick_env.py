...
from isaaclab_tasks.manager_based.motion_tracking.g1.motion_tracking_env import keypts_deviation_ref_l2, joint_deviation_ref_l1, position_tracking_error, orientation_tracking_error, target_orientation_error, target_ref, root_below_threshold, root_angle_below_threshold, current_time_enc
import numpy as np
...
class G1Rewards(G1RewardsBase):
    ...
    if TRACKING:
        ...
        target_orientation_error_right = RewTerm(func=target_orientation_error,
            params={"asset_cfg": SceneEntityCfg("robot", body_names=["right_wrist_yaw_link"])},
            weight=-1.0)

        target_orientation_error_left = RewTerm(func=target_orientation_error,
    ...
    if TASK_DENSE:
        ...
        object_approach_reward_right = RewTerm(func=object_approach_reward_right,
            ...
            "offset_y": -0.2},
            weight=100.0)
        object_approach_reward_left = RewTerm(func=object_approach_reward_left,
            params={"asset_cfg": SceneEntityCfg("robot", body_names=["left_wrist_yaw_link"]),
            "offset_x": 0.,
            "offset_y": 0.2},
            weight=100.0)
    ...
    if TASK_SPARSE:
        object_above_threshold = RewTerm(
            ...
            weight=1.0,
            params={"height_thres": 0.7, "fall_thres": 0.66}
        )
...
class ObservationsCfg:
    ...
    class PolicyCfg(ObsGroup):
        ...
        left_hand_pose_val = ObsTerm(func=hand_pose, params={"asset_cfg": SceneEntityCfg("robot", body_names=["left_wrist_yaw_link"])})
        
        target_orientation_left = ObsTerm(
            func=target_orientation, params={"asset_cfg": SceneEntityCfg("robot", body_names=["left_wrist_yaw_link"])})

        target_orientation_right = ObsTerm(
...
class MySceneCfg(MySceneCfgBase):
    platform = AssetBaseCfg(
        ...
        init_state=AssetBaseCfg.InitialStateCfg(pos=(2.5, 0, 0.25), rot=(1, 0, 0, 0)),
        spawn=sim_utils.CuboidCfg(
            size=(1., 2., 0.5),collision_props=sim_utils.CollisionPropertiesCfg(),
    ...
    object = RigidObjectCfg(
        ...
        spawn=sim_utils.CuboidCfg(
            ...
            visual_material=sim_utils.PreviewSurfaceCfg(diffuse_color=(0., 0., 0.8), metallic=0.3),
            physics_material=sim_utils.RigidBodyMaterialCfg(
                friction_combine_mode="max",
                restitution_combine_mode="min",
                static_friction=1.2,
...
@configclass
class G1BimanualPickEnvCfg(G1InteractiveBaseEnvCfg):
...
@configclass
class G1BimanualPickEnvPlayCfg(ManagerBasedRLEnvCfg):
    rewards: G1Rewards = G1Rewards()
    events: EventCfg = EventCfg()
    observations: ObservationsCfg = ObservationsCfg()
    scene: MySceneCfg = MySceneCfg(num_envs=8192, env_spacing=2.5)
    ...
    def __post_init__(self):
        ...
        rot_mat_before[:,0,:] = -rot_mat_before[:,0,:]  # flip the x-axis
        rot_after = math_utils.quat_from_matrix(rot_mat_before)
        self.scene.camera = CameraCfg(prim_path="{ENV_REGEX_NS}/Camera_new",
                                      ...
                                      offset=CameraCfg.OffsetCfg(
                                          pos=(7.348+2.0-8.82, -11.197+6.82, 1.20917),
                                          rot=rot_after[0].tolist(),
                                          convention="opengl"
                                      ),)
        self.terminations.base_contact = None
        self.scene.platform = AssetBaseCfg(
            prim_path="{ENV_REGEX_NS}/Platform",
            spawn=sim_utils.UsdFileCfg(usd_path="assets/warehouse.usd",scale=(1.,1.,1.)),
            init_state=AssetBaseCfg.InitialStateCfg(pos=(-8.82+2.0, 6.82, 0.), rot=(0, 0, 0, 1)),
        )
