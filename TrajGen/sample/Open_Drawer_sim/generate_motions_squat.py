...
def generate_ref_traj(wrist_pos, target_x = 0.55, target_z = 0.8, target_y = -0.2, door_open_amt = 0.25):
...
def compute_cost(joint_angles, trans, quats, offset_x=-0.55, offset_z=-0., debug=False):
    ...
    for link_name, tf in fk_results.items():
        if link_name == "right_wrist_yaw_link":
            ...
            angle = torch.acos(torch.clamp((rot_mat[:, 0, 0] + rot_mat[:, 1, 1] + rot_mat[:, 2, 2] - 1) / 2, -0.99, .99))
...
for i in tqdm.tqdm(range(len(SHIFT_Xs))):
    ...
    with open("../" + SAVE_FOLDER + "/" + filename ,"wb") as f:
        pickle.dump({"global_pose": Ts_world_root, "joints": joints_new, "global_position": trans_new, "open_drawer_pos": motion_data["squat_pos_real"], "grab_idx": FIRST_FRAME+40+PAUSE_AMT+INTERP_AMT}, f)
