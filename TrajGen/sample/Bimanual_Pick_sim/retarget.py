...
def main():
    ...
    hints = DATA['hint'][:1,1:]
    smpl_keypoints_ = DATA['motion'][:1].transpose((0,3,1,2)) # Shape: (k, N, 22, 3)
    ...
    for i in range(num_motions):
        ...
        global_orientation[:30] = global_orientation[30]
        global_position[:30] = global_position[30]
        jointss[i][:30] = jointss[i][30]
        ...
        with open("../"+TASK_NAME+ "/" + str(i) + ".pkl", "wb") as f:
            pickle.dump({"global_pose": global_pose , "joints": jointss[i], "global_position": global_position, "bimanual_pos": grab_pos , "grab_idx": pick_t}, f)
