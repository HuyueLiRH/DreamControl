...
grab_pos = torch.tensor(np.array(motion_data['bimanual_pos'])).clone()
...
timestep_slider = server.gui.add_slider("timestep", 0, num_timesteps - 1 + INTERP_AMT + PAUSE_AMT, 1, 0)
...
with open(SAVE_DIR + pkl_path[:-4] + "_n.pkl","wb") as f:
    pickle.dump({"global_pose": Ts_world_root , "joints": joints_new, "global_position": trans_new, "bimanual_pos": motion_data["bimanual_pos"], "grab_idx": motion_data["grab_idx"]+PAUSE_AMT+INTERP_AMT}, f)

# Define cuboid dimensions (width, height, depth)
width, height, depth = .5, 0.82, 2.0
...
transform = np.eye(4)
transform[:3, 3] = [grab_pos[0]+0.5-0.3, 0., 0.41]
