...
# SHIFT_Xs = np.concatenate(([X_MIN], SHIFT_Xs))
# SHIFT_Ys = np.concatenate(([Y_MIN], SHIFT_Ys))
# SHIFT_Zs = np.concatenate(([Z_MIN], SHIFT_Zs))
INTERP_AMT = 10
PAUSE_AMT = 10
...
def generate_ref_traj(wrist_pos, target_x = 0.45, target_z = 0.8, target_y = -0.1, door_radius = 0.32, door_open_amt = np.pi/4., hand_size=0.12):
...
def compute_cost(joint_angles, trans, quats, offset_x=-0.55, offset_z=-0., debug=False):
    ...
    # batch_angles = joint_angles  # shape (1, n_joints)
    # print(joint_angles.shape, active_joint_names)
    q_dict = {name: joint_angles[:, i] for i, name in enumerate( active_joint_names ) }
    for id in inactive_joint_ids:
        # print(id, joint_names[id])
        name = joint_names[id]
    ...
    for link_name, tf in fk_results.items():
        # print(i, link_name)
        # cost2 += 0.*torch.norm(pos - pos_ref, dim=1)  # L2 cost for position
        # print(i)
        if link_name == "right_wrist_yaw_link":
            ...
            # print((torch.abs(diff[:,0])+torch.abs(diff[:,1])+ torch.abs(diff[:,2])))
            ...
            # rot_mat_ref = torch.tensor([[torch.cos(transformed_keypts_ref[:,3]-np.pi/4.) , torch.cos(transformed_keypts_ref[:,3]+np.pi/4.), torch.zeros(transformed_keypts_ref[:,3].shape)], 
            #                             [-torch.sin(transformed_keypts_ref[:,3]-np.pi/4.), torch.sin(transformed_keypts_ref[:,3]+np.pi/4.), torch.zeros(transformed_keypts_ref[:,3].shape)], 
            #                             [torch.zeros(transformed_keypts_ref[:,3].shape), torch.zeros(transformed_keypts_ref[:,3].shape), torch.ones(transformed_keypts_ref[:,3].shape)]], dtype=torch.float32).to(rot_mat.device)  # Adjust for the coordinate system
...
for i in tqdm.tqdm(range(len(SHIFT_Xs))):
    ...
    # print("Optimized joint angles:", joint_angles.data.numpy()[45])
    # print("joint angles:", target_joint_angles.numpy()[45])
    ...
    with open("../" + SAVE_FOLDER + "/" + filename ,"wb") as f:
        pickle.dump({"global_pose": Ts_world_root, "joints": joints_new, "global_position": trans, "open_door_pos": motion_data["open_drawer_pos"], "grab_idx": 40+PAUSE_AMT+INTERP_AMT}, f)
...
while True:
    with server.atomic():
        ...
        # base_frame_new.position[0] *= 0.75
