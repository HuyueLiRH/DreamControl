...
pkl_paths = glob.glob("*.pkl")
PAUSE_AMT = 10
INTERP_AMT = 10
REDUCE_PAUSE_AMT = 0
BACK_TO_BASE_TIME = 30
SAVE_DIR = "../Button_Press_sim2/"
VIS = False
...
for pkl_path in tqdm.tqdm(pkl_paths[:]):
    ...
    button_press_t = PAUSE_AMT + INTERP_AMT 
    for i in range(button_press_t, trans_new.shape[0]-1):
        if trans_new[i, 0] == trans_new[i+1, 0] and trans_new[i, 1] == trans_new[i+1, 1] and trans_new[i, 2] == trans_new[i+1, 2]:
            button_press_t = i
            break
    if button_press_t > 130 :
        print("Warning: Pick time is too large, skipping.")
        continue
    button_press_t_end = button_press_t + 10 

    button_press_t_end_ = min(button_press_t_end + BACK_TO_BASE_TIME, trans_new.shape[0]-1)
    joints_new[button_press_t_end:button_press_t_end_, right_arm_ids] = joints_new[button_press_t_end-1:button_press_t_end, right_arm_ids].clone() + \
        (torch.tensor(init_joint_angles)[right_arm_ids].unsqueeze(0) - joints_new[button_press_t_end-1:button_press_t_end, right_arm_ids]) * \
        torch.linspace(0, 1, BACK_TO_BASE_TIME).unsqueeze(1)
    trans_new[button_press_t_end_:, :] = trans_new[button_press_t_end_-1:button_press_t_end_, :].clone()
    quats_new[button_press_t_end_:, :] = quats_new[button_press_t_end_-1:button_press_t_end_, :].clone()
    joints_new[button_press_t_end_:, right_arm_ids] = joints_new[button_press_t_end_-1:button_press_t_end_, right_arm_ids].clone()
    ...
    with open(SAVE_DIR + pkl_path[:-4] + "_n.pkl","wb") as f:
        pickle.dump({"global_pose": Ts_world_root , "joints": joints_new, "global_position": trans_new, "hit_pos": motion_data["hit_pos"], "grab_idx": motion_data["grab_idx"]+PAUSE_AMT+INTERP_AMT}, f)
