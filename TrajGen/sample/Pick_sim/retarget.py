...
def main():
    ...
    for i in range(num_motions):
        ...
        pick_t = onp.argmin(dists)
        min_dist = dists[pick_t]
        min_y = float(onp.min(smpl_keypoints_[i,:,RIGHT_HAND_INDEX,1]))
        if min_dist > 0.15:
            print("Warning: Pick distance is too far, skipping.")
            continue
        if min_y < 0.7:
            print("Warning: min height of right hand is too low, skipping.")
            continue
        ...
        if max_pos_x + 0.15 > grab_pos[0]:
            ...
            continue
        print("Pick time:", pick_t, "Min distance:", min_dist, "right_hand_pos:", right_hand_pos, "min_y:", min_y, "grab_pos:", grab_pos, torch.max(angle_with_world_x))
