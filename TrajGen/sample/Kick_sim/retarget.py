...
def main():
    ...
    for i in range(num_motions):
        ...
        kick_t = onp.argmax(global_keypts[:,RIGHT_FOOT_INDEX,0])
        
        min_z = torch.min(global_keypts[:,:,2],dim=1).values
        grab_pos = global_keypts[kick_t, RIGHT_FOOT_INDEX, :]
        global_position[:, 2] -= min_z
        grab_pos[2] -= min_z[kick_t]
        ...
        if torch.max(angle_with_world_x) > torch.pi/2.:
        ...
        global_keypts = transform_keypts(keypts, global_orientation, global_position)
        min_z = torch.min(global_keypts[:,:,2],dim=1).values
        grab_pos = global_keypts[kick_t, RIGHT_HAND_JOINT_INDEX, :]
        global_position[:, 2] -= min_z
        grab_pos[2] -= min_z[kick_t]
        if max_pos_x > grab_pos[0]:
        ...
        with open("../"+TASK_NAME+ "/" + str(i) + ".pkl", "wb") as f:
            pickle.dump({"global_pose": global_pose , "joints": jointss[i], "global_position": global_position, "kick_pos": grab_pos , "grab_idx": kick_t}, f)
