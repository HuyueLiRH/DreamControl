...
def main():
    ...
    for i in range(num_motions):
        ...
        pick_t = 0
        for joint_pos in hint:
            pelvis_pos = joint_pos[PELVIS_INDEX]
            if pelvis_pos[2] > 0. :
                break
            pick_t += 1
        ...
        grab_pos = global_keypts[pick_t, PELVIS_LINK_INDEX, :]
        ...
        with open("../"+TASK_NAME+ "/" + str(i) + ".pkl", "wb") as f:
            pickle.dump({"global_pose": global_pose , "joints": jointss[i], "global_position": global_position, "sit_pos": grab_pos , "grab_idx": pick_t}, f)
