...
def main():
    ...
    hints = DATA['hint'][:,1:]
    smpl_keypoints_ = DATA['motion'][3:4].transpose((0,3,1,2)) # Shape: (k, N, 22, 3)
    ...
    for i in range(num_motions):
        ...
        global_position[:pick_t] = global_position[pick_t:pick_t+1] + (global_position[:pick_t] - global_position[pick_t:pick_t+1]) * 0.6
        ...
        with open("../"+TASK_NAME+ "/" + str(i) + ".pkl", "wb") as f:
            pickle.dump({"global_pose": global_pose , "joints": jointss[i], "global_position": global_position, "pick_grab_pos": grab_pos , "grab_idx": pick_t}, f)
