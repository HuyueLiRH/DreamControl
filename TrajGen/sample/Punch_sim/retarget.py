...
def main():
    ...
    for i in range(num_motions):
        ...
        dists = onp.linalg.norm(smpl_keypoints_[i,:140,RIGHT_HAND_INDEX] - right_hand_pos[None,:], axis=-1)
        punch_t = onp.argmin(dists)
        ...
        grab_pos = global_keypts[punch_t, RIGHT_HAND_JOINT_INDEX, :]
        global_position[:, 2] -= min_z
        grab_pos[2] -= min_z[punch_t]
        
        print(i, punch_t, grab_pos)
        if FREEZE_FOR > 0:
            global_position[FREEZE_FOR:] = global_position[:motion_len-FREEZE_FOR].clone()
            global_orientation[FREEZE_FOR:] = global_orientation[:motion_len-FREEZE_FOR].clone()
            jointss[i][FREEZE_FOR:] = jointss[i][:motion_len-FREEZE_FOR].clone()

            global_position[:FREEZE_FOR] = global_position[0]
            global_orientation[:FREEZE_FOR] = global_orientation[0]
            jointss[i][:FREEZE_FOR] = jointss[i][0]
        
        joint_names = ['left_hip_pitch_joint', 'left_hip_roll_joint', 'left_hip_yaw_joint', 'left_knee_joint', 'left_ankle_pitch_joint', 'left_ankle_roll_joint', 'right_hip_pitch_joint', 'right_hip_roll_joint', 'right_hip_yaw_joint', 'right_knee_joint', 'right_ankle_pitch_joint', 'right_ankle_roll_joint', 'waist_yaw_joint', 'left_shoulder_pitch_joint', 'left_shoulder_roll_joint', 'left_shoulder_yaw_joint', 'left_elbow_joint', 'left_wrist_roll_joint', 'left_wrist_pitch_joint', 'left_wrist_yaw_joint', 'right_shoulder_pitch_joint', 'right_shoulder_roll_joint', 'right_shoulder_yaw_joint', 'right_elbow_joint', 'right_wrist_roll_joint', 'right_wrist_pitch_joint', 'right_wrist_yaw_joint']
        rot_matrix = quaternion_to_matrix(global_orientation)
        x_axis = rot_matrix[:,:,0]
        angle_with_world_x = torch.acos(torch.clamp(x_axis[:,0], -.99, .99))
        if torch.max(angle_with_world_x) > torch.pi/4.:
        ...
        global_position[:, 2] -= min_z
        grab_pos[2] -= min_z[punch_t]
        if max_pos_x + 0.15 > grab_pos[0]:
            print("Warning: Robot is moving forward after grabbing")
            continue
        ...
        with open("../"+TASK_NAME+ "/" + str(i) + ".pkl", "wb") as f:
            pickle.dump({"global_pose": global_pose , "joints": jointss[i], "global_position": global_position, "punch_pos": grab_pos , "grab_idx": punch_t}, f)
