...
def main():
    ...
    print(num_timesteps , is_left_foot_contact.shape  )
    ...
    for i in range(num_motions):
        ...
        button_press_t = onp.argmax(global_keypts[:,RIGHT_HAND_JOINT_INDEX,0])
        print("Button press time:", button_press_t, "keypt:", smpl_keypoints_[i,button_press_t,RIGHT_HAND_INDEX])
        
        # freeze for a few frames after button_press_t
        min_z = torch.min(global_keypts[:,:,2],dim=1).values
        grab_pos = global_keypts[button_press_t, RIGHT_HAND_JOINT_INDEX, :]
        global_position[:, 2] -= min_z
        grab_pos[2] -= min_z[button_press_t]
        motion_len = jointss[0].shape[0]
        
        global_position[button_press_t+FREEZE_FOR:] = global_position[button_press_t:motion_len-FREEZE_FOR].clone()
        global_orientation[button_press_t+FREEZE_FOR:] = global_orientation[button_press_t:motion_len-FREEZE_FOR].clone()
        jointss[i][button_press_t+FREEZE_FOR:] = jointss[i][button_press_t:motion_len-FREEZE_FOR].clone()
        
        global_position[button_press_t:button_press_t+FREEZE_FOR] = global_position[button_press_t]
        global_orientation[button_press_t:button_press_t+FREEZE_FOR] = global_orientation[button_press_t]
        jointss[i][button_press_t:button_press_t+FREEZE_FOR] = jointss[i][button_press_t]
        
        joint_names = ['left_hip_pitch_joint', 'left_hip_roll_joint', 'left_hip_yaw_joint', 'left_knee_joint', 'left_ankle_pitch_joint', 'left_ankle_roll_joint', 'right_hip_pitch_joint', 'right_hip_roll_joint', 'right_hip_yaw_joint', 'right_knee_joint', 'right_ankle_pitch_joint', 'right_ankle_roll_joint', 'waist_yaw_joint', 'left_shoulder_pitch_joint', 'left_shoulder_roll_joint', 'left_shoulder_yaw_joint', 'left_elbow_joint', 'left_wrist_roll_joint', 'left_wrist_pitch_joint', 'left_wrist_yaw_joint', 'right_shoulder_pitch_joint', 'right_shoulder_roll_joint', 'right_shoulder_yaw_joint', 'right_elbow_joint', 'right_wrist_roll_joint', 'right_wrist_pitch_joint', 'right_wrist_yaw_joint']
        rot_matrix = quaternion_to_matrix(global_orientation)
        x_axis = rot_matrix[:,:,0]
        angle_with_world_x = torch.acos(torch.clamp(x_axis[:,0], -.99, .99))
        if torch.max(angle_with_world_x) > torch.pi/3.:
            print("Warning: Angle with world x is too large, skipping.")
            continue
        keypts = get_keypts(jointss[i], joint_names , pk2_robot=pk2_robot)
        max_pos_x = torch.max(global_position[:,0])
        global_keypts = transform_keypts(keypts, global_orientation, global_position)
        min_z = torch.min(global_keypts[:,:,2],dim=1).values
        grab_pos = global_keypts[button_press_t, RIGHT_HAND_JOINT_INDEX, :]
        global_position[:, 2] -= min_z
        grab_pos[2] -= min_z[button_press_t]
        if max_pos_x + 0.05 > grab_pos[0]:
            print("Warning: Robot is moving forward after pressing button")
            continue
        ...
        with open("../"+TASK_NAME+ "/" + str(i) + ".pkl", "wb") as f:
            pickle.dump({"global_pose": global_pose , "joints": jointss[i], "global_position": global_position, "hit_pos": grab_pos , "grab_idx": button_press_t}, f)
