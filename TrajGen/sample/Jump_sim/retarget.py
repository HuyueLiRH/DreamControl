...
def main():
    ...
    for i in range(num_motions):
        ...
        min_z = torch.min(global_keypts[0,:,2])
        grab_pos = global_keypts[jump_t, RIGHT_HAND_JOINT_INDEX, :]
        ...
        with open("../"+TASK_NAME+ "/" + str(i) + ".pkl", "wb") as f:
            pickle.dump({"global_pose": global_pose , "joints": jointss[i], "global_position": global_position, "jump_pos": grab_pos , "grab_idx": jump_t}, f)
