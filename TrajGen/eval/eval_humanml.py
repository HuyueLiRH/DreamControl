...
def evaluate_matching_score(eval_wrapper, motion_loaders, file, motions_dict=None):
    match_score_dict = OrderedDict({})
    R_precision_dict = OrderedDict({})
    activation_dict = OrderedDict({})
    print('========== Evaluating Matching Score ==========')
    
    motion_names = ["pick_gt", "Pick-a", "Pick-b", "Button-Press-a", "Button-Press-b", "Ground-Pick-a", "Ground-Pick-b", "Ground-Pick-New-a", "Ground-Pick-New-b", "Bimanual-Pick-a", "Bimanual-Pick-b", "Pick-Place-a", "Pick-Place-b", "Jump-a", "Jump-b", "Punch-a", "Punch-b", "Kick-a", "Kick-b"]
    ...
    #         activation_dict[motion_loader_name] = all_motion_embeddings[5:7]

    motion_name = "button_press_gt"
    with torch.no_grad():
        motion = motions_dict[motion_name+".npy"]
        all_motion_embeddings = np.array(eval_wrapper.get_motion_embeddings(motion, torch.tensor([190]*len(motion))).cpu().numpy())
    
    match_score_dict[motion_name] = 0.#matching_score
    R_precision_dict[motion_name] = 0.#R_precision
    activation_dict[motion_name] = all_motion_embeddings.copy()

    motion_names = ["Button-Press-a", "Button-Press-b"]
    for motion_name in motion_names:
        with torch.no_grad():
            motion = motions_dict[motion_name+".npy"][:1000]
            all_motion_embeddings = np.array(eval_wrapper.get_motion_embeddings(motion, torch.tensor([190]*1000)).cpu().numpy())
...
def evaluate_fid(eval_wrapper, groundtruth_loader, activation_dict, file, filter_word="pick"):
    eval_dict = OrderedDict({})
    gt_motion_embeddings = []
    print('========== Evaluating FID ==========')
    with torch.no_grad():
        for idx, batch in enumerate(groundtruth_loader):
            _, _, captions, sent_lens, motions, m_lens, _, _ = batch
            # import pdb; pdb.set_trace()
            for i in range(len(captions)):
                caption = captions[i]
                if filter_word in caption:
                    print(caption)
                    motion_embeddings = eval_wrapper.get_motion_embeddings(
                        motions=motions[i:i+1],
                        m_lens=m_lens[i:i+1]
                    )
    ...
    # import pdb; pdb.set_trace()
    prev_mu = gt_mu.copy()
    prev_cov = gt_cov.copy()
    j = 0
    for model_name, motion_embeddings in activation_dict.items():
        mu, cov = calculate_activation_statistics(motion_embeddings)
        # print(mu, cov, gt_mu, gt_cov)
        fid = calculate_frechet_distance(gt_mu, gt_cov, mu, cov)
        print(f'---> [{model_name}] FID: {fid:.4f}')
        print(f'---> [{model_name}] FID: {fid:.4f}', file=file, flush=True)

        fid = calculate_frechet_distance(prev_mu, prev_cov, mu, cov)
        print(f'---> [{model_name}] FID: {fid:.4f}')
        print(f'---> [{model_name}] FID: {fid:.4f}', file=file, flush=True)
...
def evaluation(eval_wrapper, gt_loader, eval_motion_loaders, log_file, replication_times, diversity_times, mm_num_times, run_mm=False, motions_dict=None):
    print("Here???")
    with open(log_file, 'w') as f:
        ...
        for replication in range(replication_times):
            ...
            mat_score_dict, R_precision_dict, acti_dict = evaluate_matching_score(eval_wrapper, motion_loaders, f, motions_dict=motions_dict)
...
if __name__ == '__main__':
    ...
    test_motions_dir = "motions_humanml_test"
    motions_dict = {}
    for file in os.listdir(test_motions_dir):
        if file.endswith(".npy"):
            motions = np.load(os.path.join(test_motions_dir, file))
            motions = torch.from_numpy(motions).to(dist_util.dev())
            motions_dict[file] = motions.clone()
    ...
    evaluation(eval_wrapper, gt_loader, eval_motion_loaders, log_file, replication_times, diversity_times, mm_num_times, run_mm=run_mm, motions_dict=motions_dict)
